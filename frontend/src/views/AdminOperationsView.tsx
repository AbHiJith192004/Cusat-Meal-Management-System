  ...
  return <main className="page-container">
    ...
    {loading ? <p role="status" className="p-8 text-center">Loading…</p> : <>
      {panel==='menu' && <section className="grid lg:grid-cols-2 gap-4">
        ...
        <Card title={`Published menu for ${menuForm.date}`}><Rows empty="No menu is published for this date.">{(Array.isArray(data)?data:[]).map((x:any)=><Row key={x.id} title={x.meal_type} detail={x.items.join(', ')} meta={x.notes}/>)}</Rows></Card>
      </section>}
